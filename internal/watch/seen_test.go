...
func TestMutesSurviveARestart(t *testing.T) {
	dir := t.TempDir()
	mutes, err := LoadMutes(dir)
	if err != nil {
		t.Fatal(err)
	}
	if mutes.IsMuted("s1") {
		t.Error("a search nobody touched came back silenced")
	}

	if muted, err := mutes.Toggle("s1", "Motos"); err != nil || !muted {
		t.Fatalf("toggle left it muted=%t: %v", muted, err)
	}
	again, err := LoadMutes(dir)
	if err != nil {
		t.Fatal(err)
	}
	if !again.IsMuted("s1") || again.Count() != 1 {
		t.Fatal("the silence was not remembered")
	}
	if again.Muted["s1"].Name != "Motos" {
		t.Errorf("the name was not kept: %+v", again.Muted["s1"])
	}

	if muted, err := again.Toggle("s1", "Motos"); err != nil || muted {
		t.Fatalf("the second press left it muted=%t: %v", muted, err)
	}
	if third, _ := LoadMutes(dir); third.IsMuted("s1") {
		t.Error("giving the search back was not remembered")
	}
}

// A nil store is the plain case: nothing has ever been silenced.
func TestNilMutesSilenceNothing(t *testing.T) {
	var mutes *Mutes
	if mutes.IsMuted("s1") {
		t.Error("a nil store silenced a search")
	}
}
